...
REF_RATE_HZ        = 1000  # measured_joint_states publish rate
CAMERA_RATE_HZ     = 2.4   # effective in-bag camera rate of the old recordings

# One dataset step = one camera frame (1000 / 2.4 ≈ 417 → grid ≈ 2.4 Hz).
Downsample_factor = round(REF_RATE_HZ / CAMERA_RATE_HZ)

# x — samples per inference cycle, in dataset steps (= 1 with the values above).
ACTION_SHIFT = round(REF_RATE_HZ / CAMERA_RATE_HZ / Downsample_factor)
...
def synchronize(buckets: dict) -> dict:
    ...
    # Downsampled reference indices (1 kHz -> one step per camera frame)
    ...
    for k, i in enumerate(indices):
        ...
        # Command at time t — re-pairing with t+ACTION_SHIFT happens in shift_actions().
        gello_joints.append(extract_joints(get_at(TOPICS["gello_joints"], i)))
...
def compute_actions(synced: dict) -> np.ndarray:
    """
    Action at timestep t = GELLO joint command at t  (7,)
                         + gripper command at t       (1,)
    Combined: (T, 8)
    (Pairing with the next camera frame is done afterwards by shift_actions.)
    """
...
def shift_actions(synced: dict, actions: np.ndarray, x: int) -> tuple:
    """
    Re-pair observations with actions x steps in the future:
        observations [o1 ... on] -> [o1 ... o(n-x)]     (drop last x)
        actions      [a1 ... an] -> [a(1+x) ... an]     (drop first x)
    Row t of both outputs then holds (o_t, a_{t+x}); both have length n - x.
    """
    if x == 0:
        return synced, actions
    n = actions.shape[0]
    if n <= x:
        raise ValueError(f"Episode too short to shift: {n} steps, shift={x}.")

    obs_keys = ["joint_positions", "gripper_state", "images_primary", "images_wrist"]
    trimmed = {k: (v[:-x] if k in obs_keys else v) for k, v in synced.items()}
    return trimmed, actions[x:]
...
def main():
    ...
    for ep_idx, bag_path in enumerate(bag_dirs):
        ...
        try:
            ...
            synced, actions = shift_actions(synced, actions, ACTION_SHIFT)
